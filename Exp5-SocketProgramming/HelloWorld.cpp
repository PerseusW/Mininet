...
#define serverInterface "127.0.0.1"
...
public:
    Server(int argc, char **argv) {
        ...
        receptionSocket = getSocketDescriber(); serverAddr = getDefaultServerAddr();
        ...
        sendHelloWorld(communicationSocket);
...
private:
    ...
    struct sockaddr_in getDefaultServerAddr() {
    ...
    void sendHelloWorld(int socket) {
...
public:
    Client(int argc, char **argv) {
        resolveArguments(argc, argv); checkArguments();
        communicationSocket = getSocketDescriber();serverAddr = getDefaultServerAddr();
        connect(communicationSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr));

        showClientInfo();

        receiveMessage(communicationSocket);
...
private:
    ...
    void receiveMessage(int socket) {
        ...
        read(socket, recvBuffer, sizeof(recvBuffer));
